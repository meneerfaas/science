...
\begin{opgave}[icons={laptop}]
Maak een nieuw codebestand en sla het op als \file{turtle\_star.py}. Schrijf een programma dat een vijfpuntige ster tekent zoals in figuur~\ref{fig:turtle-star}. Je mag zelf de kleur en de pendikte bepalen, maar voor het daadwerkelijke tekenen mag je maximaal 3 regels code gebruiken.

\begin{figure}[ht]
    \centering
    \adjustimage{width=0.25\linewidth}{five_pointed_star}
...
%--------------------------------------
\begin{antwoord}
\begin{python-breakable}[style=mu-noframe, numbers=none]
for n in range(5):
    tony.fd(200)
    tony.rt(144)
\end{python-breakable}
...
\begin{subfigures}
    \begin{figure}[ht]
        \begin{minipage}[c]{0.45\linewidth}
                \adjustimage{width=0.7\linewidth, center}{flower_of_squares_2}
                \caption{Bloem van vierkanten}
                \label{fig:turtle-squares-flower}
        \end{minipage}
        \hfill
        \begin{minipage}[c]{0.45\linewidth}
                \adjustimage{width=0.7\linewidth, center}{flower_of_squares_3}
